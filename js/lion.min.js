...
function handleTouchEnd(event){mousePos={x:windowHalfX,y:windowHalfY};isBlowing=!1}
function handleTouchMove(event){if(event.touches.length==1){event.preventDefault();mousePos={x:event.touches[0].pageX,y:event.touches[0].pageY}}}
...
init();createLights();createFloor();createLion();createFan();loop();function clamp(v,min,max){return Math.min(Math.max(v,min),max)}
